<script lang="ts">
	...
	import { getFamilyEndpointPickerConfiguration } from '$lib/components/kind-viewer/kind-viewer-actions/llm-inference-service-config/templates/endpoint-picker';
...
{#if lodash.get(object, ['metadata', 'labels', 'model-template.otterscale.io/lm-cache']) === 'true'}
		...
		<Dialog.Trigger>
			{#snippet child({ props })}
				<Item.Root {...props} class="w-full p-0 text-xs" size="sm">
					...
					<Item.Content>
						<Item.Title>LM Cache</Item.Title>
					</Item.Content>
				</Item.Root>
			{/snippet}
		</Dialog.Trigger>
						...
						<div class="flex w-full items-center justify-between gap-3">
							<Button
								...
								onclick={async () => {
									...
									toast.promise(
										async () => {
											const response = await resourceClient.create({
												cluster,
												namespace: namespace,
												group: targetGroup,
											...
											const ownerUid = lodash.get(modelService, 'metadata.uid') as string;

											const ownerReference = ownerUid
												? {
														apiVersion: `${targetGroup}/${targetVersion}`,
														kind: targetKind,
											...
											const eppConfig = getFamilyEndpointPickerConfiguration(
												familyEndpointPickerParamters
											);
											lodash.set(workloadConfig, 'metadata.ownerReferences', [ownerReference]);
											lodash.set(eppConfig, 'metadata.ownerReferences', [ownerReference]);
											...
											await Promise.all([
												resourceClient.create({
													...
													manifest: new TextEncoder().encode(stringify(workloadConfig))
												}),
												resourceClient.create({
													cluster,
													namespace: namespace,
													...
													version: 'v1alpha2',
													resource: 'llminferenceserviceconfigs',
													manifest: new TextEncoder().encode(stringify(eppConfig))
												})
											]);
										},
										{
											loading: `Deploying ${targetKind} ${name}...`,
											success: () => `Successfully deployed ${targetKind} ${name}`,
											error: (error) => {
