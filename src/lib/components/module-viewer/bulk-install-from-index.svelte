<script lang="ts">
	...
	async function install(row: Row<Record<ModuleAttribute, JsonValue>>): Promise<ResultType> {
		...
		const modules: IndexModuleType[] = lodash.get(chart, 'versions', []) as IndexModuleType[];
		const latestVersion: IndexModuleType = modules[0] || ({} as IndexModuleType);
	...
	<Dialog.Content class="max-h-[80vh] overflow-auto">
		...
		<div class="space-y-2">
			{#each rows as row (row.id)}
				{@const chart = row.original.chart as unknown as IndexModuleType}
				{@const modules = lodash.get(chart, 'versions', []) as IndexModuleType[]}
