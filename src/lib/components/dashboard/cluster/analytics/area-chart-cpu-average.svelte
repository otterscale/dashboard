<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			rawData = await fetchMultipleFlattenedRange(client, query, start, effectiveEnd, STEP_SECONDS);
		} catch {
			hasError = true;
		}
		isLoading = false;
					...
					{#snippet marks({ context })}
						<defs>
							{#each context.series.visibleSeries as series (series.key)}
								...
								<linearGradient id="cpuavg_fill{key}" x1="0" y1="0" x2="0" y2="1">
						...
						{#each context.series.visibleSeries as series (series.key)}
							{@const key = series.key.replace(/\s+/g, '')}
							<Area seriesKey={series.key} {...areaProps} fill="url(#cpuavg_fill{key})" />
						{/each}
					{/snippet}
