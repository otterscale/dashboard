<script lang="ts">
	...
	async function loadData(t: number) {
		...
		hasError = false;
		try {
			const q = `topk(${t}, sum by (device) (rate(node_network_receive_bytes_total{instance=~"${fqdn}", device!="lo"}[5m])))`;
					...
					{#snippet marks({ context })}
						<defs>
							{#each context.series.visibleSeries as series (series.key)}
								...
								<linearGradient id="netrx_fill{key}" x1="0" y1="0" x2="0" y2="1">
						...
						{#each context.series.visibleSeries as series (series.key)}
							{@const key = series.key.replace(/\s+/g, '')}
							<Area seriesKey={series.key} {...areaProps} fill="url(#netrx_fill{key})" />
						{/each}
					{/snippet}
