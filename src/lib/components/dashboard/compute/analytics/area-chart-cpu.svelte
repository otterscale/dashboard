<script lang="ts">
	...
	onMount(async () => {
		const query = {
			'1m': `avg(rate(kubevirt_vmi_cpu_usage_seconds_total{name=~"${vmName}"}[1m]))`,
			'5m': `avg(rate(kubevirt_vmi_cpu_usage_seconds_total{name=~"${vmName}"}[5m]))`,
			'15m': `avg(rate(kubevirt_vmi_cpu_usage_seconds_total{name=~"${vmName}"}[15m]))`
		};
		try {
					...
					{#snippet marks({ context })}
						<defs>
							{#each context.series.visibleSeries as s (s.key)}
								...
								<linearGradient id="vmcpu_fill{key}" x1="0" y1="0" x2="0" y2="1">
						...
						{#each context.series.visibleSeries as s (s.key)}
							{@const key = s.key.replace(/\s+/g, '')}
							<Area seriesKey={s.key} {...areaProps} fill="url(#vmcpu_fill{key})" />
						{/each}
					{/snippet}
					{#snippet tooltip()}
						<Chart.Tooltip
