<script lang="ts">
	...
	onMount(async () => {
		const query = {
			[m.read()]: `sum(rate(kubevirt_vmi_storage_read_traffic_bytes_total{name=~"${vmName}"}[5m]))`,
			[m.write()]: `sum(rate(kubevirt_vmi_storage_write_traffic_bytes_total{name=~"${vmName}"}[5m]))`
		};
		try {
	...
	<Statistics.Header>
		<Statistics.Title class="h-8 **:data-[slot=data-table-statistics-title-icon]:size-6">
			<div class="flex flex-col justify-between">
				{m.disk()}
				<p class="text-sm text-muted-foreground">{m.read_and_write()}</p>
			</div>
		</Statistics.Title>
	</Statistics.Header>
					...
					{#snippet marks({ context })}
						<defs>
							{#each context.series.visibleSeries as s (s.key)}
								...
								<linearGradient id="vmdisk_fill{key}" x1="0" y1="0" x2="0" y2="1">
						...
						{#each context.series.visibleSeries as s (s.key)}
							{@const key = s.key.replace(/\s+/g, '')}
							<Area seriesKey={s.key} {...areaProps} fill="url(#vmdisk_fill{key})" />
						{/each}
					{/snippet}
					{#snippet tooltip()}
						<Chart.Tooltip
