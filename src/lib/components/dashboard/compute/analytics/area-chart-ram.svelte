<script lang="ts">
	...
	onMount(async () => {
		const query = {
			[m.domain()]: `sum(kubevirt_vmi_memory_domain_bytes{name=~"${vmName}"})`,
			[m.resident()]: `sum(kubevirt_vmi_memory_resident_bytes{name=~"${vmName}"})`,
			[m.available()]: `sum(kubevirt_vmi_memory_available_bytes{name=~"${vmName}"})`
		};
		try {
					...
					{#snippet marks({ context })}
						<defs>
							{#each context.series.visibleSeries as s (s.key)}
								...
								<linearGradient id="vmram_fill{key}" x1="0" y1="0" x2="0" y2="1">
						...
						{#each context.series.visibleSeries as s (s.key)}
							{@const key = s.key.replace(/\s+/g, '')}
							<Area seriesKey={s.key} {...areaProps} fill="url(#vmram_fill{key})" />
						{/each}
					{/snippet}
					{#snippet tooltip()}
						<Chart.Tooltip
							...
							{#snippet formatter({ name, value })}
								...
								>
									{#if value !== undefined && value !== null}
										{@const { value: cv, unit: cu } = formatCapacity(Number(value))}
