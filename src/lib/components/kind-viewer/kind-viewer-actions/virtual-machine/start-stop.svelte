<script lang="ts">
	...
	function handleAction() {
		...
		const successLabel = isRunning ? 'stopped' : 'started';
	...
	<AlertDialog.Trigger>
		{#snippet child({ props })}
			<Item.Root {...props} class="w-full p-0 text-xs" size="sm">
				...
				<Item.Content>
					<Item.Title>{isRunning ? 'Stop' : 'Start'}</Item.Title>
				</Item.Content>
			</Item.Root>
		{/snippet}
	</AlertDialog.Trigger>
		...
		<Item.Root class="p-0">
			<Item.Content class="text-left">
				<Item.Title class="text-xl font-bold"
					>{isRunning ? 'Stop' : 'Start'} Virtual Machine</Item.Title
				>
				<Item.Description>
					Are you sure you want to {isRunning ? 'stop' : 'start'} virtual machine
					<strong>{name}</strong>
