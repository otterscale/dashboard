<script lang="ts">
	...
	async function fold(editor: import('monaco-editor').editor.IStandaloneCodeEditor) {
		...
		function traverse(node: any) {
			if (isMap(node) || (node && node.items && node.constructor?.name === 'YAMLMap')) {
							...
							if (metadataIsMap) {
								metadataValues.items.forEach((subPair: any) => {
									if (isPair(subPair) && isScalar(subPair.key)) {
										const subKey = subPair.key.value;
										if (
											subKey !== 'name' &&
											subKey !== 'namespace' &&
											subKey !== 'labels' &&
...
>
	...
	<AlertDialog.Content class="min-w-[77vw]">
		...
		<div class="grid grid-cols-2 gap-4 *:max-h-[62vh] *:min-h-[62vh]">
			...
			<div class="transition-opacity duration-300 {ready ? 'opacity-100' : 'opacity-0'}">
				<Monaco
					options={{
						language: 'yaml',
						padding: { top: 24 },
