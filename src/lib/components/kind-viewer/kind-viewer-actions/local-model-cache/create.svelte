	...
	>
		...
		<Tabs.Root value={currentStep}>
			...
			<Tabs.Content value={steps[1]}>
				{#await resourceClient.list( { cluster, group: 'serving.kserve.io', version: 'v1alpha1', resource: 'localmodelnodegroups' } )}
					Loading
				{:then response}
					{@const nodeGroupNames = response.items.map((item: any) =>
					...
					<Form
						schema={{
							...
							properties: {
								sourceModelUri: {
									...lodash.get(jsonSchema, 'properties.spec.properties.sourceModelUri'),
									title: 'Source Model'
								},
								modelSize: {
									...lodash.omit(
										lodash.get(jsonSchema, 'properties.spec.properties.modelSize'),
										'anyOf'
									),
									type: 'string',
									title: 'Model Size'
								...
								nodeGroups: {
									...lodash.omit(
										lodash.get(jsonSchema, 'properties.spec.properties.nodeGroups'),
										'items'
									),
									items: {
										type: 'string',
