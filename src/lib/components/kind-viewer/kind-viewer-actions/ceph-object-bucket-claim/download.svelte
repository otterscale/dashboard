<script lang="ts">
	...
	import type { CoreV1ConfigMap, CoreV1Secret, ObjectbucketIoV1Alpha1ObjectBucketClaim } from '@otterscale/types';
	...
	<Dialog.Trigger>
		{#snippet child({ props })}
			<Item.Root {...props} class="w-full p-0 text-xs" size="sm">
				...
				<Item.Content>
					<Item.Title>Download</Item.Title>
				</Item.Content>
			</Item.Root>
		{/snippet}
	</Dialog.Trigger>
	<Dialog.Content class="max-h-[95vh] min-w-[62vw] overflow-auto">
		<Item.Root class="p-0">
			<Item.Content class="text-left">
				...
				<Item.Description>
					Use the following command to download a file from your object storage bucket. Replace the
...
SIGNATURE=$(echo -en "GET\\n\\n\\n$TIMESTAMP\\n/${bucketName}/$KEY" \\
  ...
  | base64)
curl -s "http://${endpoint}/${bucketName}/$KEY" \\
  -H "Date: $TIMESTAMP" \\
