	...
	onMount(async () => {
		try {
			const [overallRes, usingRes, totalRes] = await Promise.all([
				client.instantQuery(`sum(node_memory_SwapTotal_bytes{instance=~"${fqdn}"})`),
				client.instantQuery(
					`sum(node_memory_SwapTotal_bytes{instance=~"${fqdn}"})-sum(node_memory_SwapFree_bytes{instance=~"${fqdn}"})`
				),
				client.instantQuery(`sum(node_memory_SwapTotal_bytes{instance=~"${fqdn}"})`)
			]);
...
<Statistics.Root type="ratio">
	...
	<Statistics.Content class="min-h-20">
		...
		{:else}
			...
			{@const value = totalVal === 0 ? 0 : usingVal / totalVal}
				...
				>
					{#snippet aboveMarks()}
						{@const percentage =
							totalVal === 0
								? '0'
								: (formatPercentage(usingVal as number, totalVal as number, 1) ?? '0')}
