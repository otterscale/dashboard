<script lang="ts">
	...
	import LicenseNodes from './license-nodes.svelte';
	...
	let {
		...
		onSaveNodes: (nodes: string[]) => Promise<void>;
	...
	const nodeCount = $derived(status.nodeCount ?? 0);
...
<div class="flex h-full flex-col overflow-y-auto">
	...
	<div class="flex flex-col gap-4 p-6">
		<Tabs.Root value="overview">
			<Tabs.List>
				<Tabs.Trigger value="overview">{m.license_overview_tab()}</Tabs.Trigger>
				{#if !isProvisional}
					<Tabs.Trigger value="nodes">{m.license_authorized_nodes_tab()}</Tabs.Trigger>
				{/if}
			</Tabs.List>
								...
								<div class="flex flex-col gap-3 py-4 md:px-6 md:py-0">
									...
									{#if isProvisional}
										<span class="text-sm text-muted-foreground"
											>{m.license_fingerprint_provisional()}</span
										>
									{:else}
									...
									{#if status.bindingRefreshedAt}
										<div class="flex flex-col gap-0.5">
											<span class="text-xs text-muted-foreground"
												>{m.license_binding_refreshed_at()}</span
											>
											<span class="text-xs"
												>{new Date(status.bindingRefreshedAt).toLocaleString()}</span
											>
										</div>
									{/if}
					...
					{#if isProvisional}
						...
						>
							<Card.Header class="pb-3">
								<Card.Title
									class="flex items-center gap-2 text-base text-amber-700 dark:text-amber-400"
								>
							...
							<Card.Root>
								...
								<Card.Content class="space-y-3">
									<div class="flex items-end justify-between">
										<div>
											<p class="text-3xl font-bold">{authorizedCount}</p>
											<p class="text-xs text-muted-foreground">
												{m.license_authorized_count()} / {m.license_max_nodes()}
												{maxNodes}
											</p>
										</div>
										<div class="text-right">
											<p class="text-sm text-muted-foreground">{m.license_total_nodes()}</p>
											<p class="text-xl font-semibold">{nodeCount}</p>
										</div>
									</div>
									<Progress value={nodeProgress} class="h-2" />
									<p class="text-right text-xs text-muted-foreground">
										{authorizedCount} / {maxNodes}
										{m.license_authorized_count()}
									</p>
								</Card.Content>
							</Card.Root>
												...
												<div>
													<div class="mb-1 flex justify-between text-xs">
														...
														<span class="text-muted-foreground"
															>{gpu.authorizedCards} / {gpu.maxCards}</span
														>
													</div>
													<Progress
						...
						<Card.Root>
							<Card.Header class="pb-2">
								<Card.Title class="text-sm font-medium"
									>{m.license_node_authorizations_title()}</Card.Title
								>
							</Card.Header>
											...
											<Table.Row>
												...
												<Table.Cell class="text-xs">
													{#if node.gpus && node.gpus.length > 0}
														{node.gpus.map((g) => `${g.modelID}×${g.count}`).join(', ')}
													{:else}
														—
													{/if}
												</Table.Cell>
			...
			<!-- Authorized Nodes tab -->
			{#if !isProvisional}
				<Tabs.Content value="nodes">
					<div class="pt-4">
						<LicenseNodes
							currentNodes={license.spec?.authorizedNodes ?? []}
							lastAcceptedNodes={status.lastAcceptedAuthorizedNodes ?? []}
							{maxNodes}
							{nodeAuthorizations}
							{gpuQuota}
							overrideRejected={conditions.some(
								(c) => c.type === 'AuthorizedNodesOverrideRejected' && c.status === 'True'
							)}
							selectionMode={status.selectionMode ?? 'fifo'}
							onSave={onSaveNodes}
						/>
					</div>
				</Tabs.Content>
