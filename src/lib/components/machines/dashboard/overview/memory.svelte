<script lang="ts">
	...
	const memoryUsagesTrend = $derived(
		memoryUsages.length > 1 && memoryUsages[memoryUsages.length - 2]?.value
	...
	async function fetchTotalMemory() {
		try {
			const response = await prometheusDriver.instantQuery(
				'sum(kube_node_status_capacity{resource="memory"})'
			);
			totalMemoryBytes = response.result?.[0]?.value?.value;
		} catch {
			totalMemoryBytes = 0;
		}
	}
	...
	async function fetch() {
		await Promise.all([fetchTotalMemory(), fetchMemoryUsages()]);
	}
